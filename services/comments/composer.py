...
class HumanLikeComposerV31:
    ...
    def detect_category_and_subjects(cls, title: str, excerpt: str) -> Tuple[str, List[str], float]:
        ...
        # Subject 후보 추출 (1순위: 제목 내 키워드, 2순위: 본문 고빈도)
        subjects: List[str] = []
        if best_cat != "UNKNOWN_TOPIC":
            policy = CATEGORY_POLICIES[best_cat]
            for token in title_tokens:
                if any(k in token for k in policy.keywords) and token not in subjects:
        ...
        confidence = 0.5 + min(0.4, best_score * 0.05)
    ...
    def generate_candidates(
        ...
        if category in ("FOOD", "CAFE"):
            ...
            candidates.append(CommentCandidate(
                body=f"탁 트인 분위기가 너무 멋지네요. {primary_subj} 둘러보기 참 좋아 보여요!",
                category=category, reaction_intent=ReactionIntent.PRAISE,
        ...
        elif category in ("FITNESS", "LIFESTYLE", "WORK"):
            candidates.append(CommentCandidate(
                body=f"{primary_subj} 순서와 방식이 보기 쉽게 잘 정리되어 있어서 도움이 되네요 :)",
                category=category, reaction_intent=ReactionIntent.DETAIL_PRAISE,
                first_person_intent=FirstPersonIntent.NONE, subject=primary_subj, template_id="detail_life_1"
            ))
        else:
            candidates.append(CommentCandidate(
        ...
        # 4. PLAN_INTENT ('나'의 계획 편입)
        if category in ("FOOD", "CAFE", "TRAVEL"):
            candidates.append(CommentCandidate(
                body=f"{primary_subj} 기억해뒀다가 이쪽 가게 되면 코스에 꼭 넣어봐야겠어요 :)",
                category=category, reaction_intent=ReactionIntent.PLAN_INTENT,
                first_person_intent=FirstPersonIntent.PLAN_TO_VISIT, subject=primary_subj, template_id="plan_place_1"
            ))
            candidates.append(CommentCandidate(
                body=f"다음에 방문하게 되면 {primary_subj}도 잊지 않고 꼭 챙겨봐야겠네요!",
                category=category, reaction_intent=ReactionIntent.PLAN_INTENT,
                first_person_intent=FirstPersonIntent.PLAN_TO_VISIT, subject=primary_subj, template_id="plan_place_2"
            ))
        else:
            candidates.append(CommentCandidate(
                body=f"다음에 관련해서 찾아볼 때 {primary_subj} 내용도 함께 참고해봐야겠어요 :)",
                category=category, reaction_intent=ReactionIntent.PLAN_INTENT,
                first_person_intent=FirstPersonIntent.CURIOUS_TO_SEE, subject=primary_subj, template_id="plan_gen_1"
            ))

        # 5. PREFERENCE ('나'의 취향 공감)
        if category in ("CAFE", "INTERIOR_HOME", "TRAVEL"):
            candidates.append(CommentCandidate(
                body=f"공간이 참 편안해 보이네요. 저도 이런 아늑한 분위기 좋아해서 눈길이 가요 :)",
            ...
            candidates.append(CommentCandidate(
                body=f"전체적인 분위기가 참 마음에 드네요. 이런 깔끔한 구성 저도 취향이에요 :)",
                category=category, reaction_intent=ReactionIntent.PREFERENCE,
                first_person_intent=FirstPersonIntent.LIKE_THIS_MOOD, subject=primary_subj, template_id="pref_gen_1"
            ))
        ...
        candidates.append(CommentCandidate(
            ...
            first_person_intent=FirstPersonIntent.NONE, subject=primary_subj, template_id="empathy_1"
        ))

        # 7. INFO_REACTION (정보성 긍정 반응)
        if category in ("FOOD", "CAFE", "TRAVEL"):
            candidates.append(CommentCandidate(
                body=f"{primary_subj} 위치와 정보까지 함께 볼 수 있어서 가기 전에 참고하기 참 좋겠어요 :)",
                category=category, reaction_intent=ReactionIntent.INFO_REACTION,
                first_person_intent=FirstPersonIntent.NONE, subject=primary_subj, template_id="info_react_1"
            ))
        else:
            candidates.append(CommentCandidate(
                body=f"{primary_subj} 내용이 잘 정리되어 있어서 필요한 내용 쏙쏙 확인하기 좋네요 :)",
                category=category, reaction_intent=ReactionIntent.INFO_REACTION,
                first_person_intent=FirstPersonIntent.NONE, subject=primary_subj, template_id="info_react_2"
            ))
    ...
    def rank_and_select(
        ...
        # 점수 기준 정렬 후 상위권에서 소프트 랜덤 선택 (자연스러운 변동성)
