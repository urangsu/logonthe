...
class AIPromptBuilder:
    ...
    def build(
        ...
        if preset_str == "thoughtful":
            length_guide = "길이는 NFC 기준 공백 포함 35~95자를 기본으로 하고(2~3문장 자연스러운 호흡), 최대 150자를 넘지 마"
            rhythm_guide = """[톤 안내용 예시 — 실제 이웃 소통형 고품질 예시]
빵 종류가 다양해서 갈 때마다 뭘 골라야 할지 행복한 고민하게 되는 곳이네요! 저도 빵 좋아하는데 근처 갈 일 생기면 메모해 둬야겠어요~
새단장 이벤트 덕분에 할인된 가격으로 기분 좋게 다녀오셨네요! 매장 진열도 깔끔해서 고르는 재미가 쏠쏠해 보여요
사진 보니까 마제소바 특유의 짭짤꾸덕한 감칠맛이 제대로 느껴지네요! 계란 노른자 톡 터뜨려 비벼 먹으면 꿀맛이겠어요
통창으로 보이는 오션뷰가 시원해서 머무는 것만으로도 힐링되는 공간이네요! 날씨 좋은 날 커피 한잔하러 가보고 싶어져요
음식이 정갈하고 플레이팅도 예뻐서 대접받는 기분 들겠어요! 국물이 진하고 깔끔해 보여서 가족들이랑 가기 딱 좋겠네요"""
        elif preset_str == "calm":
            length_guide = "길이는 NFC 기준 공백 포함 25~80자를 기본으로 하고 최대 130자를 넘지 마"
...
소금빵 담백하고 너무 맛있겠네요~"""
            ...
            length_guide = "길이는 NFC 기준 공백 포함 22~65자를 기본으로 하고 최대 130자를 넘지 마"
            rhythm_guide = """[톤 안내용 예시]
...
"""
        ...
        period_rule = "1. 마침표는 문장 끝이나 연결에 자연스럽게 사용해도 좋아 (단, 로봇 같은 보고서체 금지)." if preset_str == "thoughtful" else "1. 마침표는 절대 쓰지 마 (., 。 모두 절대 금지)."

        prompt = f"""자연스러운 한국어 구어로 블로그 본문에서 직접 확인한 구체적인 세부 내용에 진정성 있게 공감하며 댓글을 작성해.
...
3. 글 전체를 단순 평가하거나 요약하지 말고, 본문에 직접 등장한 구체적인 세부 내용(메뉴, 사진 디테일, 인상적인 장면 등)에 반응해.
4. 화장실, 돌아기, 10분, 노이즈 같은 일반 단어에 '비쥬얼', '맛도리', '미쳤다'를 붙이지 마.
5. 물결표(~)는 안 써도 되며 쓰더라도 최대 2개까지만 써.
6. 헐, 와, 미쳤다, 맛도리 같은 강한 유행어는 남발하지 말고 필요할 때 자연스럽게만 써.
7. {length_guide}
...
- 전체적으로, 전반적으로, 무엇보다, 그중에서도, 인상적, 인상적이네요, 알찬 정보, 유익한 정보, 좋은 정보, 정성 가득, 한눈에, 돋보, 구성이, 완성도가
...
[출력 형식 — 중요]
설명, 따옴표, 마크다운 없이 최종 댓글만 한 줄로 출력해
